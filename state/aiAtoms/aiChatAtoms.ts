...
export const chatMessagesSelectorFamily = selectorFamily({
    ...
    get: (chatId: string) => ({get, getCallback}) => {
        ...
        const addAssistantText = getCallback(({snapshot, set}) => async (messageId: string, fullResponse: string) => {
            const {data, error} = await updateMessageText(messageId, fullResponse);
            if (error) {
                console.error('Error adding user message:', error);
            } else { console.log('addUserMessage User message added:', data); }
        });
...
export const chatStarter = selector<(userMessage: string, chatId: string) => ChatDetailsType>({
    key: 'chatStarter',
    get: ({get}) => (userMessage: string, chatId: string): ChatDetailsType => {
...
export const chatWithMessagesSelector = selectorFamily<ChatDetailsType | null, string>({
    ...
    get: (chatId: string) => async ({get}) => {
        ...
        const {
            messages,
            fetchMessages,
        } = get(chatMessagesSelectorFamily(chatId));
