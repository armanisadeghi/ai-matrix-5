...
export async function POST(req: NextRequest) {
    ...
    if (!user_id || !role || !text || (role === 'user' && !chat_id)) {
        return NextResponse.json({ error: 'user_id, role, text, and (chat_id for user messages) are required' }, { status: 400 });
    }
    ...
    if (chat_id) {
        ...
        return NextResponse.json({ error: 'title is required for creating a new chat with a system message' }, { status: 400 });
    }
}
