import { activeChatMessagesArrayAtom, chatMessagesAtomFamily, hasSubmittedMessageAtom, streamTriggerAtomFamily } from '@/state/aiAtoms/aiChatAtoms';
...
const messagePackageSelector = selector<MessageType | null>({
    ...
    get: ({get}) => {
        ...
        const messageArray = get(chatMessagesAtomFamily(activeChatId));
...
const blankSenderEntry = selector({
    ...
    get: ({get}) => {
        const activeChatId = get(activeChatIdAtom);
        if (activeChatId === null) return null;
        const activeMessages = get(chatMessagesAtomFamily(activeChatId));
