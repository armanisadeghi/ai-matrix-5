...
import { Container, Paper, Title, TextInput, Button, Group, Stack, Card, Text, Modal, ActionIcon, Tooltip, Divider, LoadingOverlay, ScrollArea, } from '@mantine/core';
import { notifications } from '@mantine/notifications';
import { IconPlus, IconEdit, IconTrash, IconMessage } from '@tabler/icons-react';
...
const CreateChatForm: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        if (title.trim().length < 3) {
            notifications.show({title: 'Error', message: 'Chat title must be at least 3 characters long', color: 'red'});
            return;
        }
        ...
        <Modal opened={!!chat} onClose={onClose} title="Update Chat">
            <form onSubmit={handleSubmit}>
                <TextInput
                    ...
                    minLength={3}
                />
                <Button type="submit" mt="md" loading={isLoading}>Update Chat</Button>
            </form>
        </Modal>
    );
};
            ...
            <Stack gap="xs">
                {chatSummaries.map((chat) => (
                    <Card key={chat.chatId} shadow="sm" p="lg" radius="md" withBorder>
                        ...
                        <Text size="sm" color="dimmed">ID: {chat.chatId}</Text>
                        <Text size="sm" color="dimmed">Created: {formatDate(new Date(chat.createdAt))}</Text>
                        <Text size="sm" color="dimmed">Edited: {formatDate(new Date(chat.lastEdited))}</Text>
                    </Card>
                ))}
            </Stack>
    );
...
export const ManageChatsComponent: React.FC = () => {
    ...
    return (
        <Container size="lg">
            ...
            <Title order={2} mb="xl">Manage Chats</Title>
            <Stack gap="xl">
                <CreateChatForm/>
