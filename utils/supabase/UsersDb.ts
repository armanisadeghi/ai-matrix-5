...
export class UsersDb {
    ...
    static async createUser(matrixUser: MatrixUserInsert): Promise<MatrixUser | null> {
        try {
            matrixUser.matrix_id = uuidv4()
            const { data, error } = await supabase.from('user').insert([matrixUser]).select('*') // Select inserted data to return it
    ...
    static async updateUserByMatrixId(
        matrixId: string,
        updatedData: Partial<MatrixUser>
    ): Promise<void> {
        try {
            const { error } = await supabase
                .from('user')
