...
const startStreaming = (chatId: string) => (callbackInterface: CallbackInterface) => {
    ...
    set(streamingUpdateSelector(chatId), {
        chatId,
        id: uuidv4(),
        role: "assistant",
        text: "",
...
const useStreamChat: React.FC<useStreamChatProps> = ({ trigger, onComplete }) => {
    ...
    const startStream = useRecoilCallback(
        (callbackInterface) => (chatId: string) => startStreaming(chatId)(callbackInterface),
        [],
    );
    const updateStream = useRecoilCallback(
        (callbackInterface) => (chatId: string, content: string) =>
            updateStreamingMessage(chatId, content)(callbackInterface),
        [],
    );
    const endStream = useRecoilCallback(
        (callbackInterface) => (chatId: string) => endStreaming(chatId)(callbackInterface),
        [],
    );
