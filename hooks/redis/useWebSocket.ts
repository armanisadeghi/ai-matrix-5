...
export const useWebSocket = (
...
): [WebSocketResponse | null, (request: RequestPayload) => void] => {
    ...
    useEffect(() => {
        ...
        const ws = new WebSocket(
            `ws://localhost:3000/api/redis/${request.channel === 'webSocketText' ? 'text' : 'data'}?channel=${request.metadata.requestId}`
        )
        setSocket(ws)
