...
import { AmeFileUploadOverComponent } from "@/ui/buttons/AmeFileUploadOverComponent";
...
const DynamicTextarea = forwardRef<HTMLDivElement, DynamicTextareaProps>(
    ({ systemText, placeholderText, className }, ref) => {
        const { userInput, handleInputChange, handleSendMessage, textareaRef, streamTrigger, setStreamTrigger } =
            useDynamicTextareaLogic();

        const { collapsed, isFocused, handleToggle, handleBoxClick } = useDynamicTextArea(() =>
