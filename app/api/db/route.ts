...
async function handleRequest(
    ...
    console.log(
        `handleRequest called with method: ${method}, table: ${table}, queryParams: ${JSON.stringify(queryParams)}, bodyParams: ${JSON.stringify(bodyParams)}`
    )
    ...
    try {
        ...
        switch (method) {
            case 'GET':
                ...
                console.log(
                    `GET request with selectColumns: ${selectColumns}, filterColumn: ${filterColumn}, filterValue: ${filterValue}`
                )
                ...
                console.log(
                    `GET response data: ${JSON.stringify(data)}, error: ${JSON.stringify(error)}`
                )
                ...
                if (data) {
                    const transformedData = data.map((record: any) =>
                        transformData(table, 'toFrontend', record)
                    )
            ...
            case 'POST':
                ...
                console.log(
                    `POST request with transformedBodyParams: ${JSON.stringify(transformedBodyParams)}`
                )
                ;({ data, error } = await supabase.from(table).insert(transformedBodyParams))

                console.log(
                    `POST response data: ${JSON.stringify(data)}, error: ${JSON.stringify(error)}`
                )
            ...
            case 'DELETE':
                console.log(
                    `DELETE request with transformedQueryParams: ${JSON.stringify(transformedQueryParams)}`
                )
            ...
            case 'PATCH':
                ...
                console.log(
                    `PATCH request with updateRecord: ${JSON.stringify(updateRecord)}, transformedQueryParams: ${JSON.stringify(transformedQueryParams)}`
                )
                ;({ data, error } = await supabase
                    .from(table)
                    .update(updateRecord)
                    .match(transformedQueryParams))

                console.log(
                    `PATCH response data: ${JSON.stringify(data)}, error: ${JSON.stringify(error)}`
                )
...
export async function POST(req: NextRequest) {
    ...
    const table = searchParams.get('table') || 'default_table'
    console.log(
        `POST request to table: ${table} with queryParams: ${JSON.stringify(queryParams)}, bodyParams: ${JSON.stringify(bodyParams)}`
    )
...
export async function DELETE(req: NextRequest) {
    ...
    const table = searchParams.get('table') || 'default_table'
    console.log(
        `DELETE request to table: ${table} with queryParams: ${JSON.stringify(queryParams)}`
    )
...
export async function PATCH(req: NextRequest) {
    ...
    const table = searchParams.get('table') || 'default_table'
    console.log(
        `PATCH request to table: ${table} with queryParams: ${JSON.stringify(queryParams)}, bodyParams: ${JSON.stringify(bodyParams)}`
    )
