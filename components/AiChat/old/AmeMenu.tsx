...
import { chatMessagesAtomFamily, chatMessagesSelectorFamily } from '@/state/aiAtoms/chatMessagesState';
import React, { useState, useContext, ReactNode } from 'react';
import { Menu, rem, Modal, TextInput, Button, Box, Space } from '@mantine/core';
...
const AmeMenu: React.FC<AmeMenuProps> & { Target: React.FC<{ children: ReactNode }> } = (
    ...
    const {messages, initialChats, onFetchMessages, onAddUserMessage, onAddAssistantText,} = useRecoilValue(chatMessagesSelectorFamily(chatId));
    const chatMessages = onFetchMessages();

    const handlePeak = async () => {
        const message = contextValue ? contextValue.message : initialValue;
        const messagesArray = useRecoilValue(chatMessagesAtomFamily(chatId));
    ...
    return (
        <>
            ...
            >
                <Box>
                    {messagesArray.map((entry, entryIndex: number) => (
