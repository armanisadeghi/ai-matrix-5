...
interface Description {
  ...
  interface RightImageLeftContentProps {
    dataProps: {
      content: Content;
      image: string;
    };
...
const RightImageLeftContent: React.FC<RightImageLeftContentProps> = ({ dataProps }) => { 
                            ...
                            <Grid justify="center" align="center" className={classes.rowGrid}>
                                <Grid.Col span={{ xs: 12, sm: 5 }} className={classes.columnContentFirst} >
                                    <div className={classes.headerWrapper}>
                                        ...
                                        <Title className={`${classes.subTitle} animate-on-scroll`} ref={(el: HTMLDivElement | null) => elementsRef.current.push(el)} order={4} >
                                            {dataProps.content.subtitle}
                                        </Title>
                                    </div>
                                    <div className="textWrapper animate-on-scroll" ref={(el: HTMLDivElement | null) => elementsRef.current.push(el)}>
                                    {
                                ...
                                <Grid.Col span={{ xs: 12, sm: 5 }} className={classes.columnContentSecond}>
                                    <div className='imgWrapper'>
                                        <Image src={dataProps.image} alt={dataProps.content.maintitle} className="anim-fade-in" />
                                    </div>
                                </Grid.Col>
                            </Grid>
                        </div>
